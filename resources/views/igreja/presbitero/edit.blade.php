...
<div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
        ...
        <div class="container">
            <form action="{{route('igreja.presbiteros.update', ['id' => $presbiteros->id] )}}" method="post">
                 @csrf
                @method('PUT')
                <div class="form-group m-2">
                    <label for="exampleInputEmail1">NOME</label>
                    <input type="text" class="form-control" value="{{$presbiteros->nome}}" id="exampleInputEmail1" name="nome" aria-describedby="emailHelp" placeholder="nome">
                </div>
                <div class="form-group m-2">
                    <label for="exampleInputEmail1">CPF</label>
                    <input type="text" class="form-control" value="{{$presbiteros->cpf}}" id="exampleInputEmail1" name="cpf" aria-describedby="emailHelp" placeholder="cpf">
                </div>
                <div class="form-group m-2">
                    <label for="exampleFormControlSelect1">CARGO</label>
                    <select name="cargo" class="form-control" id="exampleFormControlSelect1">
                        <option value="Cooperador" {{ $presbiteros->cargo == 'Cooperador' ? 'selected' : '' }}>Cooperador</option>
                        <option value="Presbitero" {{ $presbiteros->cargo == 'Presbitero' ? 'selected' : '' }}>Presbitero</option>
                    </select>
                </div>
                <div class="form-group m-2">
                    <label for="exampleFormControlSelect1">DIZIMISTA</label>
                    <select name="dizimista" class="form-control" id="exampleFormControlSelect1">
                        <option value="{{$presbiteros->dizimista}}">{{$presbiteros->dizimista}}</option>
                    </select>
                </div>
                <div class="form-group m-2">
                    ...
                    <select name="id_setor" class="form-control" id="exampleFormControlSelect1">
                        ...
                        @foreach($setores as $setor)
                            <option value="{{$setor->id}}" {{ ($presbiteros->setor->id ?? old('id_setor')) == $setor->id ? 'selected' : '' }} >{{ $setor->nome_setor }}</option>
                        @endforeach
                    </select>
                </div>
